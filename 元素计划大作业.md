...
1. 数据存储安全
   - 本地数据库加密验证
   - 登录密码加密
     ![image-20250607002723700](https://gitee.com/RSLG/typora_picture/raw/master/passowrd.png)
   - 敏感信息（令牌、密码）存储检查
2. 通信安全
   - HTTP 证书验证
   - 中间人攻击测试
...
* 项目托管在 GitHub 仓库中
* 已部署 Django 项目到远程服务器（阿里云）
* 能通过 SSH 无密码登录服务器
* 服务器已安装并配置：Python、Gunicorn、Nginx、virtualenv 等
...
├── .github/
│   └── workflows/
│       └── django.yml        ← GitHub Actions 配置文件
├── myproject/
│   └── settings.py
├── manage.py
├── requirements.txt
...
| `SERVER_USER`    | 登录服务器的用户名（如 ubuntu） |
| `SERVER_SSH_KEY` | 私钥内容（不带密码的 RSA 私钥）  |
...
2. 在 GitHub → `Actions` 页面查看工作流运行状态

3. 如果成功，将会自动部署到服务器，并重启 Gunicorn 服务
...
---
 ![image-20250607002723700](https://gitee.com/RSLG/typora_picture/raw/master/gzip.png)
